  ...
  if (!notification) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }}>
        <MaterialIcons name="notifications-off" size={64} color={colors.mediumGray} />
        <ThemedText className="mt-4 text-gray-500">Notifikation hittades inte</ThemedText>
        <TouchableOpacity onPress={() => router.back()} className="mt-6 bg-teal-500 px-6 py-3 rounded-xl">
          <ThemedText className="text-white font-semibold">Gå tillbaka</ThemedText>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <View className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-5 pt-14 pb-4">
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <ThemedText className="text-2xl font-bold flex-1">Notifikation</ThemedText>
        </View>
      </View>
      ...
      >
        ...
        {notification.details && (
          <>
            ...
            {/* Timeline */}
            {notification.details.timeline && notification.details.timeline.length > 0 && (
              <View className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 mb-4">
                <ThemedText className="text-lg font-semibold mb-4">Din dag hittills</ThemedText>
                {notification.details.timeline.map((item, index) => (
                  <View key={index} style={{ flexDirection: 'row', marginBottom: 12 }}>
                    <View style={{ marginRight: 12, alignItems: 'center' }}>
                      <View
                        style={{
                          ...
                          height: 8,
                          borderRadius: 4,
                          backgroundColor: item.isNormal ? colors.primary : '#ef4444',
                        }}
                      />
                    ...
                    <View style={{ flex: 1 }}>
                      <ThemedText className="text-sm font-semibold">{item.time}</ThemedText>
            ...
            {/* Triggers */}
            {notification.details.triggers && notification.details.triggers.length > 0 && (
              <View className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 mb-4">
                <ThemedText className="text-lg font-semibold mb-4">Faktorer</ThemedText>
                {notification.details.triggers.map((trigger, index) => (
                  <View
                    ...
                    style={{
                      ...
                      backgroundColor: trigger.active ? '#fef3c7' : colors.lightGray,
                      borderRadius: 8,
                    }}
                  >
                    <MaterialIcons
                      ...
                      style={{ marginRight: 8 }}
                    />
                    <ThemedText className="text-sm flex-1">{trigger.name}</ThemedText>
                  </View>
                ))}
              </View>
            )}
            ...
            {notification.details.actions && notification.details.actions.length > 0 && (
              <View className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 mb-4">
                <ThemedText className="text-lg font-semibold mb-4">Rekommenderade åtgärder</ThemedText>
            ...
            {notification.details.confidence !== undefined && (
              <View className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 mb-4">
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  <ThemedText className="text-sm font-semibold">Konfidensnivå</ThemedText>
                ...
                <ThemedText className="text-xs text-gray-600 dark:text-gray-400 mt-2">
                  Baserat på {notification.details.dataPoints?.length || 0} datapunkter
                </ThemedText>
              </View>
            )}

            {/* Data Points */}
            {notification.details.dataPoints && notification.details.dataPoints.length > 0 && (
